...
//		Log.d("caowj", "getSmallBitmap,缩放值scale:"+scale);
//        Log.d("caowj","scale:" + scale);
        ...
        try {
            ...
            Log.e("caowj", "图片解析异常：" + e.getMessage());
        }
    ...
    public static Bitmap getSmallBitmapByCompressedWH(Context mContext, String filePath) {
        ...
        Log.d("caowj", "本地图片上传的缩放比例ss:" + ss + ",宽：" + options.outWidth + "，高：" + options.outHeight);
    ...
    public static int calculateInSampleSize(Context mContext, BitmapFactory.Options options) {
        ...
        Log.d("caowj", "手机分辨率:宽：" + screenW + "，高：" + screenH);
        if (screenH < 1280) {
            screenH = 1280;
        }
        if (screenW < 720) {
    ...
    public static void saveCgzfPic(String localUrl, String path, String filename) {
        ...
        Log.d("caowj", "inSampleSize:" + options.inSampleSize);
        ...
        try {
            ...
            while (baos.toByteArray().length / 1024 > 50) { // 循环判断如果压缩后图片是否大于50kb,大于继续压缩
                Log.d("caowj", "length:" + baos.toByteArray().length + ",,,quality:" + quality);
                baos.reset(); // 重置baos即清空baos
            ...
            Log.d("caowj", "2length:" + baos.toByteArray().length + ",,,2quality:" + quality);
