...
//                是否包含锚点
                if (webUrl.contains(anchorFlag)) {
                    ...
                    String url2 = webUrl.substring(anchorIndex);
                    Log.d("caowj","url1:" + url1 + "-----url2:" + url2);
                    if (url1.contains("?")) {
                ...
                Log.d("caowj","防止缓存的webUrl:" + webUrl);
            } else {
                //				Log.d("caowj","支付宝链接，不能加自定义参数。："+webUrl);
            }
        }
    ...
    private static boolean isAddSurfix(String webUrl) {
        ...
        if (webUrl.contains("qipaiapp.com")) {
            //启拍的非支付页面
            flag = !webUrl.contains("/alipay/") && !webUrl.contains("/wechatpay/");
        } else {
            //非启拍页面
            flag = false;
        }
...
//        } else {
//            Log.d("caowj","bigImageUrl is null");
//        }
////		Log.d("caowj", "thumbnailUrl is "+thumbnailUrl);
//        return thumbnailUrl;
//    }
    ...
    public static float KeepTwo(String str) {
        str = "";
        Log.d("StringTool", "Json字段中金额:" + str);
        float result = Float.parseFloat(str);
        float result2 = Float.valueOf(str);
//        double result = Double.parseDouble(str);
        Log.d("StringTool", "转化成double:" + result + ",,," + result2);
        result = KeepTwoF(result);
//        result = keepTwoD(result);
        Log.d("StringTool", "保留两位:" + result);
        float d = result;
        Log.d("StringTool", "强转 double-->float:" + d);
        return d;
    }
