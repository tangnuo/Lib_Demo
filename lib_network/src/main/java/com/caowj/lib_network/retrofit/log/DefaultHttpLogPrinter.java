...
/**
 * @author : yuanbingbing
 * @since : 2018/9/11 16:03
 */
public class DefaultHttpLogPrinter implements HttpLogPrinter {
    private static final String TAG = "LegoHttpLog";
    ...
    private static final String CONTENTTYPE_TAG = "ContentType: ";
    ...
    // 是否记录到文件
    public static boolean IS_RECORD = true;
    ...
    private static String[] getRequest(Request request) {
        ...
        if (request.body() != null) mediaType = request.body().contentType();
        log = METHOD_TAG + request.method() + DOUBLE_SEPARATOR +
                (isEmpty(header) ? "" : HEADERS_TAG + LINE_SEPARATOR + dotHeaders(header))
                + (mediaType == null ? "" : CONTENTTYPE_TAG + LINE_SEPARATOR + dotHeaders(mediaType.toString()));
    ...
    /**
     * 打印网络请求信息,  {{@link okhttp3.RequestBody}} 为 {@code null} 或不可解析的情况
     *
     * @param request
     */
    @Override
    ...
    /**
     * 打印网络响应信息, {{@link okhttp3.ResponseBody}} 可以解析的情况
     *
    ...
    public void printJsonResponse(long chainMs, boolean isSuccessful, int code, String headers, MediaType contentType,
        ...
        bodyString = HttpContentTypeUtil.isJson(contentType) ? TextUtil.jsonFormat(bodyString)
                : HttpContentTypeUtil.isXml(contentType) ? TextUtil.xmlFormat(bodyString) : bodyString;
    ...
    /**
     * 打印网络响应信息, {{@link okhttp3.ResponseBody}} 为 {@code null} 或不可解析的情况
     *
     * @param chainMs      服务器响应耗时(单位毫秒)
     * @param isSuccessful 请求是否成功
     * @param code         响应码
